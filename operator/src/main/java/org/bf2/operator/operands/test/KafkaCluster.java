 ...
 * and checking the corresponding status For testing purpose only, it puts the
 * Kafka declaration into a ConfigMap so the actual Kafka cluster is not created
 */
@ApplicationScoped
...
public class KafkaCluster extends org.bf2.operator.operands.KafkaCluster {
    ...
    protected Kafka cachedKafka(ManagedKafka managedKafka) {
        ConfigMap cm = informerManager.getLocalConfigMap(kafkaClusterNamespace(managedKafka),
                kafkaClusterName(managedKafka));
        if (cm == null)
            return null;
