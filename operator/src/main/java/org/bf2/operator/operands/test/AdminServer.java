 ...
 * ManagedKafka one and checking the corresponding status For testing purpose
 * only, it puts the Deployment declaration into a ConfigMap so the actual pod
 * is not created
 */
@ApplicationScoped
...
public class AdminServer extends org.bf2.operator.operands.AdminServer {
    ...
    protected Deployment cachedDeployment(ManagedKafka managedKafka) {
        ConfigMap cm = informerManager.getLocalConfigMap(adminServerNamespace(managedKafka),
                adminServerName(managedKafka));
        if (cm == null)
            return null;
